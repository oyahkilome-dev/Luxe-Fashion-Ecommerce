...
app.post("/api/orders/process-payment", async (req, res) => {
  ...
  try {
    ...
    const { data: existingPayment } = await supabaseAdmin.from("payments").select("id").eq("reference", reference).single();
    if (existingPayment) {
    ...
    const orderNumber = `ORD-${Math.floor(Math.random() * 1e6)}`;
...
app.get("/api/admin/orders", async (req, res) => {
  try {
    ...
    const { data: orders, error } = await supabaseAdmin.from("orders").select("*, order_items(*), payments(*)").order("created_at", { ascending: false });
